...
const asyncEndpoints: EndpointSection[] = [
  {
    ...
    description: 'List public battles for an async game. Supports cursor-based pagination. Only available for games with the "async" capability.',
    auth: false,
    requestBody: {
      fields: [
        { name: 'cursor', type: 'string', required: false, description: 'Pagination cursor (battleId to start after)' },
        { name: 'limit', type: 'number', required: false, description: 'Max results (default: 20, max: 100)' },
        { name: 'status', type: 'string', required: false, description: 'Filter by status: pending, active, completed' },
      ],
      example: { limit: 20, status: 'pending' },
    },
    responseBody: {
      fields: [
        { name: 'success', type: 'boolean', description: 'Whether the request was successful' },
        { name: 'battles', type: 'Battle[]', description: 'Array of public battles' },
  ...
  },
  {
    ...
    method: 'POST',
    path: '/api/[gameSlug]/battles',
    description: 'Create a new battle. The authenticated player becomes player 1.',
    auth: true,
    requestBody: {
      fields: [
  ...
  },
  {
    ...
    method: 'POST',
    path: '/api/[gameSlug]/battles/[id]/turns',
    description: 'Submit a turn with actions. Only the current active player can submit. Must include end_turn action.',
    auth: true,
    requestBody: {
      fields: [
        { name: 'actions', type: 'Action[]', required: true, description: 'Array of actions (must end with end_turn)' },
        { name: 'gameState', type: 'object', required: false, description: 'Game state snapshot after turn (max 50KB)' },
      ],
      example: {
        actions: [
          { type: 'move', unitId: 'u1', from: { x: 1, y: 2 }, to: { x: 3, y: 2 } },
    ...
    responseBody: {
      fields: [
        ...
        { name: 'turn', type: 'Turn', description: 'Submitted turn' },
        { name: 'battle', type: 'Battle', description: 'Updated battle state' },
        { name: 'message', type: 'string', description: 'Human-readable message' },
      ],
      example: {
        success: true,
