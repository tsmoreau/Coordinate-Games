...
export default async function PlayerProfilePage({ params }: Props) {
  ...
  const [battles, scores] = await Promise.all([
    hasAsync ? getPlayerBattles(player.deviceId, game.slug, 10) : Promise.resolve([]),
    hasLeaderboard ? getPlayerScores(player.deviceId, game.slug, 20) : Promise.resolve([]),
  ]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
                    ...
                    {battles.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {battles.map((battle) => (
                          <Link 
                            key={battle.battleId} 
                            href={`/${game.slug}/matches/${encodeURIComponent(battle.displayName)}`}
                            className="block"
                          >
            ...
            {hasLeaderboard && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>This player&apos;s scores on the leaderboard</CardDescription>
                </CardHeader>
                <CardContent>
                  {scores.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                    ...
                    <div className="space-y-2">
                      {scores.map((score) => (
                        <Link
                          key={score.id}
                          href={`/${game.slug}/score/${score.id}`}
                          className="block"
                        >
                          <div className="flex items-center justify-between p-3 rounded-lg border border-border hover:border-foreground/20 hover:bg-muted/50 transition-all">
                            <div className="flex items-center gap-3">
                              <span className="text-2xl font-bold font-mono">
                                {score.score.toLocaleString()}
                              </span>
                              {score.category !== 'default' && (
            ...
            {!hasAsync && !hasLeaderboard && (
              <Card>
                <CardContent className="text-center py-12 text-muted-foreground">
                  <p>This player is registered for {game.name}.</p>
                </CardContent>
              </Card>
            )}
